...
def npySplitAndSave(npyMatrix, path, counter, isLabel=False):
    if findSliceNumPosition(npyMatrix) == 0:
        return npySplitAndSave0(npyMatrix, path, counter, isLabel)
    elif findSliceNumPosition(npyMatrix) == 2:
        return npySplitAndSave2(npyMatrix, path, counter, isLabel)

def npySplitAndSave0(npyMatrix, path, counter, isLabel=False):
...
def npySplitAndSave2(npyMatrix, path, counter, isLabel=False):
...
if __name__ == '__main__':
    ...
    for fileName in fileNames:
        ...
        # the volume data after the last training volume data will be considered as the testing volume data
            ...
            npySplitAndSave(testRawNpy, TEST_IMAGE_PATH, 0)
            testMaskNpy = np.load(MASK_VOLUME_PATH + DIR_DIVIDER + fileName)
            npySplitAndSave(testMaskNpy, TEST_LABEL_PATH, 0, isLabel=True)
            break
